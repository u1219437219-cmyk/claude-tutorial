...
更推荐的做法是：把技术栈附录理解成“通用规则的细化层”，不只是补说明，还要补规则。

例如：

- React / Next.js 附录里，可以补：
  - 组件和页面边界怎么拆
  - 状态管理优先级
  - 样式和响应式约束
  - 前端 review 的额外重点

- NestJS / Spring Boot / Express 附录里，可以补：
  - controller / service / repository 的职责边界
  - DTO / schema / validation 的约束
  - 错误处理、日志、事务和缓存的规则
  - 后端 review 的额外重点

- Python / FastAPI 附录里，可以补：
  - 路由、依赖注入、schema、service 分层约定
  - 类型检查、异步边界、异常处理方式
  - 测试和验证要求

一句话说：

- 通用 `CLAUDE.md` 管“所有项目都适用的规则”
- 技术栈附录管“只有这类项目才需要的细化规则”

这样能避免把所有技术细节都塞进主模板，同时又能保证 AI 在具体技术栈下拿到足够明确的执行口径。
